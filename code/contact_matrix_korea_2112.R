...
data = read.csv('(Kstat) 코로나19 이후 접촉도 변화 조사_1차(21년 12월)_Data.csv',fileEncoding = 'euc-kr')
library(stringr)
library(dplyr)

colnames(data)
contact_num_index = seq(from=43,157,by=6) # maximum 20

#############################################################################
# Age, Weekday/Weekend, Number of contact, Age group of contactees, Mask time
contact_data = data[,c(10,15,41,contact_num_index,22)] 
...
for( i in 1:10){
  ...
  for(j in 1:10){
    if(j !=10){
      phi[i,j] = sum(sapply(1:nrow(temp),function(x) length(which(temp[x,4:23]==j))))/nrow(temp)
      phi_week[i,j] = sum(sapply(1:nrow(temp_week),function(x) length(which(temp_week[x,4:23]==j))))/nrow(temp_week)
      phi_weekend[i,j] = sum(sapply(1:nrow(temp_weekend),function(x) length(which(temp_weekend[x,4:23]==j))))/nrow(temp_weekend)
      
      phi_mask_low[i,j] = sum(sapply(1:nrow(temp_mask_low),function(x) length(which(temp_mask_low[x,4:23]==j))))/nrow(temp_mask_low)
      phi_mask_low_week[i,j] = sum(sapply(1:nrow(temp_mask_low_week),function(x) length(which(temp_mask_low_week[x,4:23]==j))))/nrow(temp_mask_low_week)
      phi_mask_low_weekend[i,j] = sum(sapply(1:nrow(temp_mask_low_weekend),function(x) length(which(temp_mask_low_weekend[x,4:23]==j))))/nrow(temp_mask_low_weekend)
      
      phi_mask_high[i,j] = sum(sapply(1:nrow(temp_mask_high),function(x) length(which(temp_mask_high[x,4:23]==j))))/nrow(temp_mask_high)
      phi_mask_high_week[i,j] = sum(sapply(1:nrow(temp_mask_high_week),function(x) length(which(temp_mask_high_week[x,4:23]==j))))/nrow(temp_mask_high_week)
      phi_mask_high_weekend[i,j] = sum(sapply(1:nrow(temp_mask_high_weekend),function(x) length(which(temp_mask_high_weekend[x,4:23]==j))))/nrow(temp_mask_high_weekend)
...
setwd('C:/Users/Hwichang Jeong/Desktop/corona_정리/corona/Kstat_contact_matrix/Kstat')
write.csv(phi,'./contact matrix/IBUKA/2021/contact_2112_IBUKA.csv')
write.csv(phi_mask_low,'./contact matrix/IBUKA/2021/contact_2112_IBUKA_mask_low.csv')
write.csv(phi_mask_high,'./contact matrix/IBUKA/2021/contact_2112_IBUKA_mask_high.csv')
write.csv(phi_res_sb,'./contact matrix/Sebastian/2021/contact_2112_Sebastian.csv')
write.csv(phi_res_sb_mask_low,'./contact matrix/Sebastian/2021/contact_2112_Sebastian_mask_low.csv')
write.csv(phi_res_sb_mask_high,'./contact matrix/Sebastian/2021/contact_2112_Sebastian_mask_high.csv')
